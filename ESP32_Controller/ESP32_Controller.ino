...
public:
  LGFX(void) {
    ...
    bcfg.spi_host   = VSPI_HOST; // Using VSPI for standard ESP32
    bcfg.spi_mode   = 0;
    bcfg.freq_write = 27000000;
    bcfg.freq_read  = 16000000;
    bcfg.pin_sclk   = 18; // Standard VSPI SCK
    bcfg.pin_mosi   = 23; // Standard VSPI MOSI
    bcfg.pin_miso   = 19; // Standard VSPI MISO
    bcfg.pin_dc     = 2;  // Standard GPIO for DC
    _bus_instance.config(bcfg);
    _panel_instance.setBus(&_bus_instance);

    auto pcfg = _panel_instance.config();
    pcfg.pin_cs   = 15; // Standard GPIO for CS
    pcfg.pin_rst  = 4;  // Standard GPIO for RST
    pcfg.pin_busy = -1;
...
void setup() {
  ...
  Serial.println("\n\nESP32 HTTP Camera Client Starting...");
